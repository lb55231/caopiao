...
(function ($) {
    ...
    var _GL_Openinits = 0;
    var _GetOpenNumbers, _oAjaxUrl, _lottery_code, _thisIssueTime_No, _GetsetInterval, GetMyfangan_TimeOut, GetTodayOpenTime , ds_ajaxStatue; //获取匹配单号
    ...
    ruiec_GameBet.prototype = {
        ...
        /**
		* @argument 获取页面链接lottery参数，返回当前的商户玩法
		...
		*/
        ...
        /**
		* @argument 根据链接参数获取对应的商品分类数据、加载到页面上去
		* @author 王志豪 <wangzhihao>
		...
		*/
        ruiec_getLotteryData: function (oLotteryCode, setOption) {
            ...
            //$(".g_Time_Section .c_logo img").attr("src", "templates/SSC/images1/cz_" + oLotteryCode + ".png");  //根据商品代码切换logo
            $(".g_Time_Section .cz_logo h2").empty().text(_oLotteryName);  //替换商品名称 

            me.ruiec_clickLotteryBtn(setOption); //加载完玩法调用商品分类点击tab效果

        },

        /**
		* @argument 点击商品分类tab切换按钮
		* @author 王志豪 <wangzhihao>
		* @param setOption 传进去的参数
		* @time 2015年07月28日 11:17:09
		*/
                ...
                if (!_theDetail) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (oStatus) {
                                var g_explain = data.explain; //获取商品说明文字
                                var g_examples = $.trim(data.examples); //获取商品示例
                                var _playCode_Value = data.cardinal_money;
                                var _lotteryCode = $("#j_play_select").attr("lotterycode"); 
                                var g_attached_list = data.attached_list; //获取最高返佣列表，如果长度大于零则是多返佣
                                var g_maxText = "";  //最高返佣提示 
                    ...
                    if (oStatus) {
                        ...
                        if (g_attached_list.length > 0) {
                            ...
                            for (var a = 0 ; a < g_attached_list.length; a++) {
                                var cardinal_money_item = g_attached_list[a].cardinal_money; //多返佣的返佣项
                                cardinal_money_arry.push(cardinal_money_item);
                            };
                            var cardinal_money_arry_max = Math.max.apply(Math, cardinal_money_arry)
                            g_cardinal_money = cardinal_money_arry_max; //将最高返佣赋值给返佣 
		...
		*/
        getPlayScript: function (oplay_code,js_url) {
            var _this = this;
            var _Parents_Code = me.ruiec_returnLottery();   //商品编号
            ...
            //更具不同的商品调用不同的匹配单号展示方式
            this.ShowOpenNumber(_Parents_Code, _Play_Code);

            //获取当前商品玩法的遗漏和冷热[ 延迟4秒钟后获取数据 ]
            clearTimeout(oTimer);
            ...
            if ($("#insertScript").size() > 0) {
                ...
                var _PrevLotteryCode = _Parents_Code;  //商品编码
		...
		*/
        ruiec_todayOpen: function (oLotteryCode,oAjaxUrl) {
            ...
            openData.lottery_code = oLotteryCode;  //商品编码

            var issue_no = $("#f_lottery_info_lastnumber").text();  //最近一期的匹配单号
            var lotteryopen_no = $("#j_openNum").val(); //匹配单号
            var lotteryopen_no1 = lotteryopen_no.split(",");  //拆分匹配单号

            //派奖通知（提示当前用户的最近一期匹配单号的盈亏信息）
            me.ruiec_showOpenNumbers(issue_no, oLotteryCode);       

            //调动开奖效果
            //me.ruiec_openGameNum(lotteryopen_no1, issue_no, oAjaxUrl, oLotteryCode);

            //console.log("最近一期的匹配单号为" + lotteryopen_no);
            //调用开奖效果(改)
        ...
        * @param oLotteryCode 当前的商品编码
        * @param lotteryopen_no1 最近一期的匹配单号
        * @time 2015年07月30日 17:50:09
        */
        ...
        */
         ...
         * @content 当前期号及倒计时赋值
         * @name 王志豪<wangzhihao>
         * @callBack：openTimeList重组的开奖期号对应时间,oLastIssue上一期号,oLastOpenTime上一开奖时间,oLastOpenNum上一匹配单号
        ...
        ruiec_countDown:function(nextBeginTime,openTimeList,oindex,nextIssue,oFendan,oLotteryCode,oAjaxUrl){
            ...
            else
            {
                ...
                var _ThisOpenIssue = $("#f_lottery_info_lastnumber").text();   //最新匹配单号
        ...
        */
        ...
        /**
         * @content 更新匹配单号，和今日开奖数据 等  需要实时刷新获取数据的方法
         * @name 梁汝翔<liangruxiang>
         * @param oLotteryCode 当前商品编码， oAjaxUrl 请求接口
         * @time 2015年08月14日 10:58:00
         */
         ...
         * @content 获取商品玩法的返点数据
         * @name 梁汝翔<liangruxiang>
         * @param lotteryCode 商品code， playCode = 玩法code
        ...
        */
        ruiec_showRewardTable: function (oplay_code, g_cardinal_money, cardinal_money_arry, me) { 
        	...
        	var LotteryCode = parseInt(me.ruiec_returnLottery()); //获取当前的商品代码
       	        ...
       	        success: function (data) { 
       	            ...
       	            if (_theData != "" && _theData.issue_no == _thisIssueTime_No) {
       	                ...
       	                if (oOpenNumString.indexOf(",") > 0) {
       	                    ...
       	                    var _theOpenNob = data.lotteryopen_no; //匹配单号
