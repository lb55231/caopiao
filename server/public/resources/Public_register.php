...
<div class="login_main">
	<div class="login_bg" style="height:620px">
		<div class="login_title">
			<h2 class="margin_0" style="padding-bottom: 30px;">用户注册</h2>
			<p style="display:none">{$mingren}都已经注册了，赶紧加入吧！注册后系统将随机分配一张个性的头像。</p>
		</div>
		<div class="login_input">
			<form method="post" id="form1" class="ruivalidate_form_class" checkby_ruivalidate  onSubmit="return check_form(this)"  action="{:U('Public/register')}">
				...
				<!--<if condition="$defaulttjcode neq 0">
					...
					<dl style="padding:0;margin-bottom: 30px;margin-top: -20px;">
						<dd style="font-size: 12pt;color: #e4393c;padding: 20px 50px;border-bottom: 1px solid #e4393c;display: inline-block;">密码注册</dd>
					</dl>
				...
				<div class="login_pass user_commom_style position_r">
					...
					<img class="v_code" src="{:U('Public/verify',array('imageW'=>130,'imageH'=>40,'fontSize'=>18))}" onclick="this.src=this.src+'?temp='+ 1" style="cursor:pointer;right: 262px;"  alt="验证码" />
				</div>
				
					<div class="login_user user_commom_style">
					<span>邀请码：</span>
						<present name="linkinfo"><input type="hidden" name="linkid" value="{$linkinfo.id}"  /></present>
						<input type="text" <if condition="$tgid neq ''">readonly</if> value="<if condition="$tgid neq ''">{$tgid}<else/><if condition="$defaulttjcode neq 0">{$defaulttjcode}</if></if>" id="reccode" class="input fadeIn delay1"
				...
				<div class="btn_submit pull-left" >
					<input  type="submit" id="submit" class="btn btn-danger register" style="width:8em;height: 2.5em;font-weight: 700;" value="立即注册"/>　
					<input  type="reset"  style="width:8em;height: 2.5em;font-weight: 700;display:none" class="btn btn-default reset"  value="重置" />　<!--已有账号？<a href="{:U('Public/login')}" class="remmber_pwd">立即登录</a>--></p>
				</div>
			</form>
		</div>
	</div>
