			...
			<div class="vip_info_content betRecord_main">	
				<div class="betRecord_tab clearfix">
					...
					<div class="bet_status pull-left" id="dealRecord">
						...
						<span class="bet_common_bor"><a href="{:U('Account/dealRecord3')}">回款记录</a></span>
					</div>
				</div>
						...
						<volist name="mxlist" id="vo">
							<tr>
								...
								<td><?php
									...
									if(strpos($vo['remark'],'和值大') !== false){ 
 echo str_replace("和值大","精品",$vo['remark']);
}elseif(strpos($vo['remark'],'和值小') !== false){ 
echo str_replace("和值小","普货",$vo['remark']);
}elseif(strpos($vo['remark'],'和值单') !== false){ 
echo str_replace("和值单","单/件",$vo['remark']);
}
elseif(strpos($vo['remark'],'和值双') !== false){ 
echo str_replace("和值双","双/件",$vo['remark']);}
else{
	echo $vo['remark'];
}
