...
$(".play_select_tit > li").click(function(){
	...
	case 'HZ'://和值
	  ...
	  $(".play_select_prompt span").text('投注说明：至少选择1个和值投注，选号与开奖的三个号码相加的数值一致即中奖。返佣1.88-149倍');
	  break;
	case '3THTX'://三同号通选
	  $(".choice_cound").css({'display':'none'});
	  $(".play_select_prompt span").text('10元购买6个三同号(111,222,333,444,555,666)投注，选号与匹配单号一致即中奖31倍。');
	  break;
	case '3THDX'://三同号单选
	  $(".choice_cound").css({'display':'block'});
	  $(".play_select_prompt span").text('至少选择1个三同号投注，选号与匹配单号一致即中奖149倍。');
	  break;
	case '3BTH'://三不同号
	  $(".choice_cound").css({'display':'block'});
	  $(".play_select_prompt span").text('至少选择3个号码投注，选号与匹配单号一致即中奖28倍。');
	  break;
	case '3LHTX'://三连号通选
	  $(".choice_cound").css({'display':'none'});
	  $(".play_select_prompt span").text('10元购买4个三连号（123、234、345、456）投注，选号与匹配单号一致即中奖8倍。');
	  break;
	case '2THFX'://二同号复选
	  $(".choice_cound").css({'display':'block'});
	  $(".play_select_prompt span").text('10元购买1个二同号(11*,22*,33*,44*,55*,66*)投注，选号与匹配单号一致即中奖11倍。');
	  break;
	case '2THDX'://二同号单选
	  $(".choice_cound").css({'display':'block'});
	  $(".play_select_prompt span").text('选择1个相同号码和1个不同号码投注，选号与匹配单号一致即中奖51倍。');
	  break;
	case '2BTH'://二不同号
	  $(".choice_cound").css({'display':'block'});
	  $(".play_select_prompt span").text('至少选择2个号码投注，选号与匹配单号一致即中奖6倍。');
	  break;
	}
})
...
function sendorder(order,price){
	...
	strVar += "		<li><label for=\"question1\" class=\"ui-label\">商品：<\/label><span class=\"ui-text-info\">"+configs.title+"<\/span><\/li>";
	...
	/*strVar += "		<li><label for=\"question2\" class=\"ui-label\">温馨提示：本平台每单最高返佣限额<i class='c_red'>" + Max_Values + "</i>元，请会员谨慎投注！<\/li>";*/
	strVar += "	<\/ul>";
