...
(function ($, ruiec_GameBet) {
    ...
    Lottery_Basic_Fn.prototype = {
        ...
        GetBasicLotteryData: function (play_code) {
            ...
            Basic_LotteryData.LotteryName = "安徽快三";  //商品名称
            Basic_LotteryData.LotteryCode = "1301";    //商品编号
            Basic_LotteryData.LotteryType = "EasyFrame.Model.dt_lottery_play_detail";  //商品模型
        ...
        ReverseSelectionBall: function () {
            ...
            $("#order_table tbody tr").unbind("click").click(function () {
                var theTr = $(this);
                var _the_order_type_code = theTr.attr("order_type_code");  //商品编码 
            ...
            $("#f_submit_order").unbind("click").click(function (event) {
                ...
                if (this_state_btn == undefined || this_state_btn == "true") {
                    ...
                    if (_theResult.length > 0) {
                        ...
                        alert("当前商品的：" + PlayCodes + "的玩法，投注注数超出了相应的单方案最大投注量。");
                        return false;
                    } 
        ...
        Normal_SubTzOrder: function (_btnObj) {
        	...
        	if(parseFloat($("#f_gameOrder_amount").text())>0 && $("#order_table tr").size()>0)
        	{
	        	...
	        	var strVar = "";
				    ...
				    strVar += "		<li><label for=\"question1\" class=\"ui-label\">商品：<\/label><span class=\"ui-text-info\">"+comfire_tit+"<\/span><\/li>";
        ...
        GetTzOrderInfos: function (_OrderContainer, statesub) {
            ...
            _OrderContainer.find("tr").each(function () {
                if (statesub) {
                    var _TrObj = {};
                    var _theTrObj = $(this); //当前行 
                    var _Line_Lottery_code = _theTrObj.attr("order_type_code");  //商品
        ...
        */
         ...
         * @content 获取开奖时间列表
         * @name 王志豪<wangzhihao>
         * @param oLotteryCode是当前商品编码，oAjaxUrl获取到的ajax提交路径
         * @time 2015年08月13日 16:58:00
         */
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data)
                    {
	                    ...
	                    var oLastOpenNum = data.open_num; //最后一期的匹配单号
