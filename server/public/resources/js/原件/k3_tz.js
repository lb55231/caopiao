		...
		var html = '';
		    ...
		    tmpCodes.each(function(i){
		        ...
		        for(var j=0;j<count ;j++){
		        	...
		        	var val = $(this).val() ;
		        	 addTotal = addTotal + parseInt(eachPrice , 10); // 累计下单金额
		        	var tr = '';
		        	if( j%2 == 0){ 
		        		tr = '<tr >';
		        	} else {
...
//                CP.core.addBall(code,val,_type);
    ...
    checkSingleBuyNew : function(that){
        ...
        if(l>0 && val<l){
            CP.core.checkSingleTip('您的单注最低投注额为 '+_chipin_l+' 元，但您目前的下单金额为 '+val+' 元，请修改！',that);
            return false;
        }
        if(h>0 && val > h){
            CP.core.checkSingleTip('您的单注最高投注额为 '+_chipin_h+' 元，但您目前的下单金额为 '+val+' 元，请修改！',that);
            return false;
        }
    },
    ...
    checkSingleBuy:function(that){
        ...
        if(l>0 && val<l){
            CP.core.checkSingleTip('您的单注最低投注额为 '+user.lowest+' 元，但您目前的下单金额为 '+val+' 元，请修改！',that);
            return false;
        }
        if(h>0 && val > h){
            CP.core.checkSingleTip('您的单注最高投注额为 '+user.highest+' 元，但您目前的下单金额为 '+val+' 元，请修改！',that);
            return false;
        }
    },
 	                    ...
 	                    success: function(json){
 	                        ...
 	                        switch(json.tip){
								...
								case 'timeout':
									...
									break;
 	                            case 'success':
 	                                CP.tip('您的投注信息已经成功提交，请待匹配！【<a href="'+myLotteryUrl+'">查看我的购买信息</a>】');
 	                                CP.core.clearAll();
 	                                CP.zhui.clearAll();
...
// 	                               $(".expand").toggle();
 	                            ...
 	                            case 'error' :
 	                            	...
 	                            	break;
 	                            case 'money' :
 	                            	CP.tip('您的余额不足，请待匹配！【<a href="'+myLotteryUrl+'">查看我的购买信息</a>】');
 	                            	break;
 	                           case 'token':
	        ...
	        if(_chipin_l>0 && totals < _chipin_l){
	            CP.tip('您的投注最低限额为“'+_chipin_l+'”元，但您目前的下单金额为“'+totals+'”元，请修改！');
	            return false;
	        }
	        if(_chipin_h>0 && totals > _chipin_h){
	            CP.tip('您的投注最高限额为“'+_chipin_h+'”元，但您目前的下单金额为“'+totals+'”元，请修改！');
	            return false;
	        }
