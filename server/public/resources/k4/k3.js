...
$(function(){
	...
	$(".menu1").eq(0).unbind().click(function () {
		$("#fn_getoPenGame").toggle(); 
	});
	...
	$(document).on('click','a.ball_number,div.ball_number', function () {
		...
		var len = $('#Game_CheckBall').find('a.curr,div.menuActive').length;
		if(len <= 0){
...
$(document).on('click','.betting_right_btn',function () {
	...
	var lottery_number_str = '';
	$('#lottery_sum_old_b').find('.lottery_sum_old').each(function () {
	    var lottery_sum = $(this).attr('lottery_sum');
	    if(lottery_sum == '大') {
	        lottery_number_str = lottery_number_str + '精品&nbsp;|&nbsp;' ;
	    } else if(lottery_sum == '小') {
	        lottery_number_str = lottery_number_str + '普货&nbsp;|&nbsp;' ;
	    } else if(lottery_sum == '单') {
	        lottery_number_str = lottery_number_str + '单/件&nbsp;|&nbsp;' ;
	    } else if(lottery_sum == '双') {
	        lottery_number_str = lottery_number_str + '双/件&nbsp;|&nbsp;' ;
	    } else {
	        lottery_number_str = lottery_number_str + lottery_sum + '&nbsp;|&nbsp;' ;
	    }
		lottery_number = lottery_number + lottery_sum + '/' ;
	})
	var html = '';
	html += '<div class="clearfix"><div class="pull-left" style="width:100px;">'+lottery.shortname+':</div><div class="pull-left" way-data="showExpect.shortname">'+lottery.currFullExpect+'单</div></div>';
	html += '<div class="clearfix"><div class="pull-left" style="width:100px;">下单金额:</div><div style="margin-left:0; text-align: left;white-space:normal;word-break:break-all;word-wrap:break-word:;"><span style="color:red;">'+lottery_moneyOld+'</span>元</div></div>';
	//html += '<div class="clearfix"><div class="pull-left" style="width:100px;">下单内容:</div><div style="margin-left:100px; text-align: left;white-space:normal;word-break:break-all;" id="ordertotalprice">'+lottery_number+'</div></div>';
	html += '<div class="clearfix"><div class="pull-left" style="width:100px;">下单内容:</div><div style="margin-left:100px; text-align: left;white-space:normal;word-break:break-all;" id="ordertotalprice">'+lottery_number_str+'</div></div>';
	$.modal({
            text: html,
            buttons: [ 
...
function lottery_empty() {
	...
	$('.Choice_Ball_Container').find('.ball_number').removeClass('curr').removeClass('menuActive');
	maxRates = 0;
	ratesArr = [];
	$('.lottery_footer_sum').hide();
	$('.lottery_inputBox').hide();
...
function lottery_yuji_input(num){
	var _ratearray = [];
	var inputVal = num;//$('.lottery_input').val()/1;
	var zhushu = 0;
	var totalmoney = 0;
	var okmoney = 0;
	var _rate = 0;
...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname}, function(data) {
		if(data.sign==true){
			lottery = data.data;
			if((lottery.lastFullExpect + '').substr(0, 4) == new Date().getFullYear()) {
			    lottery['lastShortExpect'] = lottery.lastFullExpect.substr(4);
			} else {
			    lottery['lastShortExpect'] = lottery.lastFullExpect;
			}
			...
			var currFullExpect = lottery.currFullExpect;
			if((lottery.currFullExpect + '').substr(0, 4) == new Date().getFullYear()) {
			    currFullExpect = lottery.currFullExpect.substr(4);
			}
			...
			if (lottery.remainTime && eval(lottery.remainTime) > 1) {
				...
				if (ret) {
					...
					$("[way-data='showExpect.currFullExpect']").text(currFullExpect);
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
						...
						var sum = Number(openCodes[0]) + Number(openCodes[1]) + Number(openCodes[2]);
						if(sum > 10) {
						    $('#opencode').html('精品');
						} else {
						    $('#opencode').html('普货');
						...
						//$('#opencode').html(msg.data.opencode)
						//$('#opencode2').html(msg.data.opencode2)
		...
		success: function(data) {
			...
			$.each(data, function(index, val) {
				...
				if(val.isdraw == -1) { // 未匹配订单绿色
					html += '<div class="item-after"><t style="color:green">未匹配订单</t></div>';
				} else if(val.isdraw == 1) { // 已匹配订单红色
					html += '<div class="item-after"><t style="color:red">已匹配订单</t></div>';
				...
				html += '</div>';
				html += '<div class="item-subtitle">';
				html += '金额:' + val.amount + ',注数:' + val.itemcount + ',中返佣:' + (val.okamount ? val.okamount : 0) + ',中注数:'+ val.okcount;
				html += '</div>';
				html += '</div>';
				html += '</li>';
