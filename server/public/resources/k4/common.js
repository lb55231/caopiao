	...
	$.post(url, {'trano':trano}, function(data) {
		if(data.sign==true){
			if(data.BillInfo.isdraw==1){
				data.BillInfo.state='<t style="color:green">已匹配订单</t>';
			}else if(data.BillInfo.isdraw==-1){
				data.BillInfo.state='<t style="color:red">未匹配订单</t>';
			  ...
			  var html = '';
			  html += '<div class="clearfix"><div class="pull-left" style="width:100px;">商品:</div><div class="pull-left">'+data.BillInfo.cptitle+'</div></div>';
