$(function () {
  ...
  $('.bet_filter_box').on('click','.bet_options',function (){
    ...
    switch(_thisType){
      case 'pl3zxfs':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('每位至少选择一个号码，竞猜匹配单号，号码和位置都对应即中奖，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
          gameNumber(fc3d_3x_sxzhixfsq);
        break;
      case 'pl3zxds':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
      ...
      case 'pl3hzzx':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('至少选择一个和值，竞猜匹配单号数字之和，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
        gameNumber(fc3d_3x_zhixhzqs,27);
        break;
      case 'pl3kd':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('所选数值等于匹配单号的最大与最小数字相减之差，即为中奖，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
      ...
      case 'pl3zux3':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从0-9中选择2个数字组成两注，所选号码与匹配单号相同，且顺序不限，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
        gameNumber(fc3d_3x_sxzuxzsq);
        break;
      case 'pl3zux6':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从0-9中任意选择3个号码组成一注，所选号码与匹配单号相同，顺序不限，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
        gameNumber(fc3d_3x_sxzuxzlq);
        break;
      case 'pl3zuxhh':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
          .html('手动输入号码，3个数字为一注，所选号码与匹配单号相同，顺序不限，即为中奖，返佣  <em style="color:red;">'+rates.maxjj+'</em>元');
        danshiNumberL = 3;
        danshiGame();
        break;
      case 'pl3zuxbd':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从0-9中任意选择1个包胆号码，匹配单号的后三位中任意1位与所选包胆号码相同(不含豹子号)，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
        gameNumberZxbd(fc3d_3x_zuxcsbd);
        break;
      case 'pl3zsds':
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('手动输入号码，3个数字为一注，所选号码与匹配单号相同，顺序不限，返佣 <em style="color:red;">'+rates.maxjj+'</em>元');
        danshiNumberL = 3;
        danshiGame();
        break;
      case 'pl3zlds':
      ...
      case 'pl3q2zxbd': case 'pl3h2zxbd':
        if(_thisType == 'pl3q2zxbd'){
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从0-9中任意选择1个号码，匹配单号的前二位中任意1位包含所选的包胆号码相同，返佣   <em style="color:red;">'+rates.maxjj+'</em>元');
        }else if(_thisType == 'pl3h2zxbd'){
          $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从0-9中任意选择1个号码，匹配单号的后二位中任意1位包含所选的包胆号码相同，返佣   <em style="color:red;">'+rates.maxjj+'</em>元'); 
        }
        gameNumberZxbd(ex_zsxbd);
        break;
      case 'pl3dwdfs':
        $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从百位、十位、个位任意位置上至少选择1个号码，选号与相同位置上的匹配单号一致，返佣   <em style="color:red;">'+rates.maxjj+'</em>元');
        gameNumber(fc3d_1x_fs);
        break;
      case 'dxdsq2':
        $('.play_select_prompt').find('span[way-data="tabDoc"]')
            .html('从百位、十位中的“大、小、单、双”中至少各选一个组成一注，返佣  <em style="color:red;">'+rates.maxjj+'</em>元');
