...
var str = `<div id="popup_box" style="width: 100%;height: 100vh;display:none; z-index: 1000; margin: 0px;padding: 0px;top: 0px;left: 0px; position: fixed; background-color: rgba(55,55,55,.6);">
		<div style="width: 320px;height: 450px;text-align: center; margin: 0 auto;margin-top: 50vh;position: relative; top: -225px;">
			...
			<span id="popup_text" style="font-size: 16px;font-weight: 700;top: -50px;position: relative; color: #ffdc37;">恭喜您匹配成功了，详情请查看下单记录哦！</span>
		</div>
	</div>`;
	$('body').append(str);
...
var addtotouzhu = function(_playid,number,zhushu,isshow){
	...
	var price = parseInt($('.multxNumber').val());
...
function lottery_price(num){
	...
	$('#f_gameOrder_lotterys_num').html(zhushu)
	$('#f_gameOrder_amount').html(totalmoney);
	//$Detailnode.find('input.orderprice').val(num);
}
/*投注订单提交*/
$(document).on("click", "#f_submit_order", function() {
	...
	var Orderdetaillist = '';
	var Orderdetaillist2 = '<table border="1" cellspacing="0" class="table-transfer" style="width: 488px; text-align: center; border-color: rgb(153 153 153 / 30%); color: rgb(81, 90, 110);"><thead ><tr style="background-color: rgb(248, 248, 249); height: 40px;"><th class="title-game_cname" style="width: 11%;text-align:center">商品</th><th class="title-showString" style="width: 11%;text-align:center">匹配内容</th><th class="title-amountWithMode" style="width: 11%;text-align:center">金额</th></tr></thead> <tbody>';
	...
	for (var i = 0; i < orderList.length; i++) {
			...
			if(Number(cur_order.zhushu)<1 || Number(cur_order.price)<1){
				...
				return false;
						...
						var number = '';
						if(cur_order.number == '大') {
							number = '精品';
						} else if(cur_order.number == '小') {
							number = '普货';
						} else if(cur_order.number == '单') {
							number = '单/件';
						} else if(cur_order.number == '双'){
							number = '双/件';
						} else {
							number = cur_order.number;
						}
						
			Orderdetailtotalprice += oprice;
			Orderdetaillist +="<p>"+rate.title+':<span class="mark">'+cur_order.number+'</span>&nbsp;&nbsp;件:<span class="mark">'+cur_order.zhushu+'</span>&nbsp;&nbsp;金额:<span class="mark">'+oprice.toFixed(2)+"</span></p>";
			Orderdetaillist2 +='<tr style="height: 45px;"><td style="text-align:center">'+lottery.shortname+'</td><td style="word-break: break-all;text-align:center">'+number+'</td><td style="text-align:center">'+oprice.toFixed(2)+'</td></tr>';
	}
	Orderdetaillist2 += '</tbody></table><div class="otherInfo"><p class="sum">Bet<span>'+orderList.length+'</span>件，总金额：<span>￥'+Orderdetailtotalprice+'</span></p></div>';
	$("#Orderdetaillist").html(Orderdetaillist);
	$("#Orderdetailtotalprice").text(Orderdetailtotalprice.toFixed(2));

	            artDialog({
	        		//title:"下单详情<span style='margin-left:15px;'><img src='"+WebConfigs["ROOT"]+"/resources/images/icon/icon_09.png'>截至时间:<strong class='sty-h gametimes' style='font-weight:normal'>00:00:00</strong></span>",
	        		title:"匹配详情",
	        		content:Orderdetaillist2,//$("#submitComfirebox").html(),
	        		cancel:function(){},
	        		okVal:'匹配',
	        		ok:function(){
					if(!user){
...
/*					$.post(apirooturl + 'cpbuy',{"orderList":orderList,'expect':lottery.currFullExpect,'lotteryname':lotteryname}, function(json){
	        		...
	        		lock:true,
	        		init:function (w){
	        		    this.DOM.wrap.addClass('order_dialog');	
	        		}
	        	});
});
...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname/*,'cptype':'ssc'*/}, function(data) {
		if(data.sign==true){
			...
			if(yIndex > 1){
				loginCengBoxFn(lottery.lastFullExpect+'单已截止<br />当前单号<span style="color:red;">'+lottery.currFullExpect+'</span><br />下单时请注意单号');
				setTimeout(function(){
					$('.loginCengBox').hide();
				},4000);
			}
			...
			for(var o in lotlist){
				...
				if(sum%2!=0)
					...
					oddeven='<em class="bg_blue">双</em>';*/
						...
						if(sum%2!=0) {
							oddeven='<em class="bg_green">单</em>';
							$('.open_danshuang').text('单/件');
						} else {
							oddeven='<em class="bg_blue">双</em>';
							$('.open_danshuang').text('双/件');
						}
         ...
         success:function(msg){
            if (msg.sign) {
                if (msg.isOpen) {
                  if (msg.prizeMoney > 0) {
                       $(".touzhuzj ul").empty();
                        var insertHtml = "<li>投注商品：<span>" + msg.showName + "</span></li>";
                        insertHtml += "<li>投注期号：<span>" + msg.expect + "</span></li>";
                        insertHtml += "<li>下单金额：<span>" + msg.totalMoney + "元</span></li>";
                        insertHtml += "<li>收益金额：<span>" + msg.prizeMoney + "元</span></li>";
